...
def kimball_logic():
    ...
    instructions = ("Instructions:\n"
                    "1. Select the Award File for Kimball.\n"
                    "2. Select the BOM file for Kimball.\n"
...
def process_and_merge_files(parent_window):
    ...
    try:
        ...
        column_formats = {
            'AO': '"$"#,##0.0000',  # Currency values with two decimal places
            'AR': '"$"#,##0.0000',  # Currency values with two decimal places
            'AU': '0.00%'  # Percentage format
        }
...
def process_first_file(sheet):
    ...
    required_headers = ['Total EAU', 'FINAL AWARD COST', 'MOQ']
    ...
    new_headers_formulas = {
        ...
        'Ext Award Value': f"={get_column_letter(header_column_map['Total EAU'])}{{row}}*{get_column_letter(header_column_map['FINAL AWARD COST'])}{{row}}",
        'Award Conf': None,
        'EAU': f"={get_column_letter(header_column_map['Total EAU'])}{{row}}",
        'Award Price': f"={get_column_letter(header_column_map['FINAL AWARD COST'])}{{row}}",
        ...
        'Award MOQ': f"={get_column_letter(header_column_map['MOQ'])}{{row}}",
...
def merge_columns(sheet1, working_copy, columns_to_merge):
    ...
    for col in range(1, sheet1.max_column + 1):
        if sheet1.cell(row=2, column=col).value == 'KIMBALL PN':
